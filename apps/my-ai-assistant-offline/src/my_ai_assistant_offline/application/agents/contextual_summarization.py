...
class ContextualSummarizationAgent:
    ...
    def __init__(
        ...
        if not self.mock and self.api_key is None:
            raise RuntimeError(
                "An API key is required for real contextual summarization"
            )
        ...
        for chunk in chunks:
            try:
                if self.mock:
                    ...
                    new_chunk = fake_summary + chunk
                    results.append(new_chunk)
                    continue

                result = self._summarize(content, chunk)
                if result:
                    results.append(result + "\n" + chunk)
                else:
    ...
    def _summarize(
        ...
        prompt = f"""
    You are a helpful assistant specialized in summarizing documents relative to a given chunk.
    ...
    Provide a short, succinct context of no more than {self.max_characters} characters that situates this chunk within the overall document to improve search retrieval.

    Return only the context in plain text, with no headings, labels, or commentary.
    """.strip()

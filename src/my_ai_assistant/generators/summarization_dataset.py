...
from my_ai_assistant.agents.summarization import SummarizationAgent
from my_ai_assistant.domain.dataset import InstructDataset, InstructDatasetSample
...
class SummarizationDatasetGenerator:
    def __init__(self,
        ...
        if min_document_length < 1:
            raise ValueError(
                "min_document_length must be at least 1"
            )
        ...
        if max_summary_characters < 1:
            raise ValueError(
                "max_summary_characters must be at least 1"
            )

        if max_summary_length_factor <= 0:
            raise ValueError(
                "max_summary_length_factor must be positive"
            )
        ...
        self.pregeneration_filters: list[DocumentFilter] = [
            lambda document: (
                ...
                >= self.min_document_length
            ),
            lambda document: (
                document.content_quality_score is not None
                and document.content_quality_score
        ...
        self.postgeneration_filters: list[DocumentFilter] = [
            lambda document: (
                ...
                <= int(
                    ...
                    * self.max_summary_length_factor
                )
            ),
        ]
    ...
    documents: list[Document],
        ...
        for document in documents:
            if document.summary is None:
                raise ValueError(
                    f"Document {document.id} has no summary"
                )
